...
export default {
  async fetch(request, env, ctx) {
    ...
    console.log(
      `域名头部: ${hostTop} 反代地址: ${hub_host} searchInterface: ${fakePage}`
    );
    // 更改请求的主机名
    ...
    if (
      ...
      // 提取镜像名
      let repo = "";
      const v2Match = url.pathname.match(
      ...
      if (repo) {
        ...
        // 如果环境变量中有 Docker 账号密码，则注入认证信息
        if (env.DOCKER_USER && env.DOCKER_PAT) {
          // Cloudflare Workers 自带 btoa 函数用于 Base64 编码
          const authString = btoa(`${env.DOCKER_USER}:${env.DOCKER_PAT}`);
          tokenHeaders["Authorization"] = `Basic ${authString}`;
          // console.log("Using Authenticated Token Request");
        } else {
          // console.log("Using Anonymous Token Request");
        }
        ...
        if (tokenRes.status === 429) {
          return new Response(
            JSON.stringify({
              detail:
                "Rate limit exceeded from upstream (Docker Hub). Please configure DOCKER_USER and DOCKER_PAT in Workers settings.",
              error: true,
            }),
        ...
        let original_response = await fetch(
        ...
        if (new_response_headers.get("Location")) {
          ...
          console.info(
            `Found redirection location, redirecting to ${location}`
          );
